...
async def tool_repo_summary(input: RepoSummaryInput) -> RepoSummaryOutput:
    ...
    if input.tool_name and not _is_github_url(input.url):
        log.info(f"Non-GitHub URL provided ({input.url}), attempting catalog lookup for tool: {input.tool_name}")
        try:
            catalog_url = _get_repo_url_from_catalog(input.tool_name)
            if catalog_url:
                log.info(f"Found GitHub URL in catalog for {input.tool_name}: {catalog_url}")
    ...
    try:
        summary, truncated = _clip(repocards.get_repo_info(effective_url, github_token=os.getenv("GITHUB_TOKEN")))
        return RepoSummaryOutput(
            truncated=truncated,
            ref=None,
            summary=summary,

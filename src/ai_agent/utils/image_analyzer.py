# perception/image_analyzer.py
from __future__ import annotations
import base64, io, logging, mimetypes, os, json
...
def _pil_to_png_bytes(im) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(f"Pillow not installed: {e}")
...
def _nii_to_png_bytes(path: str) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(f"Nibabel not installed for NIfTI support: {e}")
    img = nib.load(path)
    data = img.get_fdata()
...
def analyze_image_with_text(image_path: str, user_task: str) -> Dict[str, Any]:
    ...
    try:
        ...
        log.warning("OpenAI SDK not available (%s); skipping VLM analysis.", e)
        return {}

"""Frontend-agnostic request handlers for the agent chat loop.

These functions contain all the business logic for processing user messages,
running the agent, executing tools, and assembling replies. They operate on
plain Python types (``ChatState``, ``ChatMessage``, dicts, strings) and have
**no dependency on any UI framework**.

Any frontend (Gradio, FastAPI, CLI, etc.) can call ``respond()`` and
``execute_tool_with_approval()`` directly.
"""
...
from ai_agent.core.chat_state import ChatState, ChatMessage
from ai_agent.core.formatters import format_tool_card
from ai_agent.core.model_config import get_model_config
...
def respond(
    ...
    This is the primary entry-point for any frontend to interact with the
    agent.  It accepts plain Python types and returns framework-agnostic
    ``ChatMessage`` / ``ChatState`` objects.

    Args:
        message: User's text message
        files: List of uploaded files (paths or file-like objects)
    ...
    if model:
        ...
        base_url_override = model_cfg.get("base_url")  # Can be None for OpenAI

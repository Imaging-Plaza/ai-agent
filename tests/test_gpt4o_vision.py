...
try:
    ...
    if result.usage:
        ...
        if hasattr(usage, 'image_tokens') and usage.image_tokens:
            ...
            print(f"   ✅ No image_tokens (expected for text-only)")
...
try:
    ...
    print(f"✅ Created multimodal prompt with {len(user_prompt)} parts (1 text + 1 image)")
    print()
    ...
    if result.usage:
        usage = result.usage()
        print(f"📊 Usage: total={usage.total_tokens}, "
        ...
        if hasattr(usage, 'image_tokens') and usage.image_tokens:
            ...
            if usage.details and 'image_tokens' in str(usage.details):
                print(f"   ✅✅✅ IMAGE CONFIRMED: Found in details")
                image_detected = True
        ...
        if usage.input_tokens >= 350:  # Accounts for image processing
            print(f"   ✅ High input token count suggests image was processed")
            image_detected = True
    ...
    negative_phrases = ["no image", "not attached", "can't see", "cannot see", "didn't receive"]
    response_lower = result.output.what_i_see.lower()
    
    has_negative = any(phrase in response_lower for phrase in negative_phrases)
    
    if result.output.image_received and result.output.confidence > 80 and not has_negative:

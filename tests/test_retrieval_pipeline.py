...
def pipeline():
    ...
    pipeline = RAGImagingPipeline(
        index_dir=str(index_dir), min_results=5, max_retries=2
    )
...
class TestMedicalRequests:
    ...
    def test_lung_segmentation_ct(self, pipeline):
        ...
        log.info(
            f"Top result: {top_doc.name} (rerank: {results[0].get('rerank_score', 'N/A')})"
        )

        # Should find lung-related tools
        result_names = [r["doc"].name.lower() for r in results]
        assert any(
            "lung" in name for name in result_names
        ), "Should find lung-related tools"
    ...
    def test_brain_mri_registration(self, pipeline):
        ...
        for i, r in enumerate(results[:3]):
            log.info(
                f"  {i+1}. {r['doc'].name} (rerank: {r.get('rerank_score', 'N/A')})"
            )
...
class TestNonMedicalRequests:
    ...
    def test_deblurring_restoration(self, pipeline):
        ...
        for r in results[:3]:
            log.info(
                f"  Restoration: {r['doc'].name} (score: {r.get('rerank_score', 'N/A')})"
            )

    def test_jpeg_format_hint(self, pipeline):
        """Test: JPEG image processing."""
        results = pipeline.retrieve("process photo", image_paths=["photo.jpg"], top_k=5)
...
class TestVaguePreciseSpectrum:
    ...
    def test_vague_segment(self, pipeline):
        ...
        result_names = [r["doc"].name.lower() for r in results]
        assert any(
            "segment" in name for name in result_names
        ), "Should find segmentation tools"

    def test_precise_3d_liver_segmentation_dicom(self, pipeline):
        """Test: Very precise request with multiple constraints."""
        results = pipeline.retrieve(
            "3D liver segmentation from DICOM CT scans using deep learning", top_k=5
        )
...
class TestOutOfCatalogRequests:
    ...
    def test_video_editing(self, pipeline):
        ...
        log.info(
            f"Video editing query returned: {[r['doc'].name for r in results[:3]]}"
        )
...
class TestRetrievalModes:
    ...
    def test_retrieve_with_rerank(self, pipeline):
        ...
        for r in results[:3]:
            log.info(
                f"  Reranked: {r['doc'].name} (rerank: {r.get('rerank_score', 'N/A')})"
            )
    ...
    def test_rerank_improves_precision(self, pipeline):
        ...
        for i in range(min(3, len(no_rerank), len(with_rerank))):
            log.info(
                f"  {i+1}. {no_rerank[i]['doc'].name} → {with_rerank[i]['doc'].name}"
            )
    ...
    def test_exclusion_filter(self, pipeline):
        ...
        filtered = pipeline.retrieve(
            "segment image", top_k=5, exclusions=[excluded_name]
        )
...
class TestImageMetadataIntegration:
    ...
    def test_format_hint_dicom(self, pipeline):
        ...
        results = pipeline.retrieve(
            "visualize scan", image_paths=["scan.dcm", "scan2.dicom"], top_k=5
        )
    ...
    def test_format_hint_nifti(self, pipeline):
        ...
        results = pipeline.retrieve(
            "view brain volume", image_paths=["brain.nii.gz"], top_k=5
        )
    ...
    def test_format_hint_tiff_stack(self, pipeline):
        ...
        results = pipeline.retrieve(
            "analyze microscopy images", image_paths=["cells.tif"], top_k=5
        )
    ...
    def test_multiple_formats(self, pipeline):
        ...
        results = pipeline.retrieve(
            "register images", image_paths=["scan1.dcm", "scan2.nii.gz"], top_k=5
        )
...
class TestEdgeCases:
    ...
    def test_very_long_query(self, pipeline):
        ...
        log.info(
            f"Long query top result: {results[0]['doc'].name if results else 'None'}"
        )
...
class TestRetryMechanism:
    ...
    def test_obscure_term_retry(self, pipeline):
        ...
        results = pipeline.retrieve(
            "analyze perfusion BOLD fMRI hemodynamic response", top_k=5
        )
